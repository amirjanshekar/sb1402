...
def nextShortPathV0(agent, MG, T):
    ...
    if a.NextNode == -1 and a.CurrentNode != a.EndNode:  ##try if neighbors are null !!! end node = nest node?
        ...
        for node in a.MapG.neighbors(str(a.CurrentNode)):
            print("log1")
            if a.MapG[str(a.CurrentNode)][node]["havlen"] < havlen and a.MapG[str(a.CurrentNode)][node][
                "seen"] < 3:  ## be node i ke oomade barnagare !!! neveshte nashode !!! baraye node flag gozashe she
                print("log1-1")
                neighbor = node
        ...
        if T * a.Velocity < havlen:
            ...
            a.MapG[str(a.CurrentNode)][a.NextNode]['havlen'] = a.MapG[str(a.CurrentNode)][a.NextNode][
                                                                   'havlen'] - T * a.Velocity
        ...
        if T * a.Velocity < havlen:
            a.MapG[str(a.CurrentNode)][str(a.NextNode)]['havlen'] = a.MapG[str(a.CurrentNode)][str(a.NextNode)][
                                                                        'havlen'] - T * a.Velocity
        elif T * a.Velocity > havlen:
            a.Path += [a.NextNode]
            G[str(a.CurrentNode)][str(a.NextNode)]["traffic"] = G[str(a.CurrentNode)][str(a.NextNode)]["traffic"] - 1
...
def nextShortPath(agent, MG, T):  ###start & end +path
    ...
    if a.NextNode == -1 and int(a.CurrentNode) != int(a.EndNode):  ##try if neighbors are null !!! end node = nest node?
        ...
        if T * a.Velocity < havlen:
            ...
            a.MapG[str(a.CurrentNode)][a.NextNode]['havlen'] = a.MapG[str(a.CurrentNode)][a.NextNode][
                                                                   'havlen'] - T * a.Velocity
            G[str(a.CurrentNode)][str(a.NextNode)]["traffic"] = G[str(a.CurrentNode)][str(a.NextNode)][
                                                                    "traffic"] + trafficC
