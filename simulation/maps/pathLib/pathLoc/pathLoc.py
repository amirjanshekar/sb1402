    ...
    for c in range(startC):
        ...
        if False:
            for eC1 in range(endC):
                if endCap[eLId]<=(startC/endC)+1:
                    d1 = locLib.difLoc(G.nodes[str(startId[c])]['lat'] ,G.nodes[str(startId[c])]['lon'] ,G.nodes[str(endId[eLId])]['lat'] ,G.nodes[str(endId[eLId])]['lon'])
                    break
                else:
                    eLId +=1
        ...
        if False:
            for ec in range(endC):
                d2 = locLib.difLoc(G.nodes[str(startId[c])]['lat'] ,G.nodes[str(startId[c])]['lon'] ,G.nodes[str(endId[ec])]['lat'] ,G.nodes[str(endId[ec])]['lon'])
                    ...
                    if a3 <= (5.2/7) :## bale
                        ...
                        if a4 <= (2.8/7) :##bale
                            ...
                            if a5 <= (4.4/7):## sabok
                                ...
                                if nx.has_path(G,str(startId[c]),str(endId[0])):
                                    agnt.append(agent.Agent(G, startId[c], -1 , endId[0] , 13, 4, [str(startId[c])], [], 0, c))
                                print("maghsad bohran az rah sabok 4")
                            elif a5 > (4.4/7):## sangin
                                ### maghsad bohran az rah sangin 
                                if nx.has_path(G,str(startId[c]),str(endId[0])):
                                    agnt.append(agent.Agent(G, startId[c], -1 , endId[0] , 13, 5, [str(startId[c])], [], 0, c))
